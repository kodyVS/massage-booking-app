...
**Brand palette** (- coral / periwinkle / cream):
...
**Landing page (`/`)**:
- Hero: business name (Fraunces), tagline, the brand artwork (placeholder image OK - use `next/image` with a stable filename so it's swappable), prominent "Book Now" CTA → `/book`
